...
Logout writes independent secret-free `oauth.json.logout` and `oauth.json.state` tombstones, then attempts and verifies deletion from both credential locations even if either marker write fails. Incomplete cleanup is reported and retried before any future load; only a newly-authorized device session may retire a completed logout tombstone. Keep `cookies.jar` and any fallback `oauth.json` safe: either can contain authorization material that gives access without the account password. Mutable application data is stored per user in `%LOCALAPPDATA%\\TwitchDropsMiner` on Windows, `${XDG_DATA_HOME:-~/.local/share}/TwitchDropsMiner` on Linux, and `~/Library/Application Support/TwitchDropsMiner` on macOS.
