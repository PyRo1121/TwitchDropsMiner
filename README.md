...
If a native provider is unavailable, the miner fails closed unless an existing fallback proves prior eligibility or the user explicitly starts it with `--allow-insecure-oauth-file`. That opt-in is persisted before a provenance-marked, versioned, mode-0600 `oauth.json` fallback may be created. Absence of a sidecar is never treated as permission because it can also mean an older native-vault installation was upgraded during an outage. After native storage has been provisioned, an outage always fails closed. A differing fallback and vault value is preserved as an explicit conflict rather than choosing and deleting one. Vault access or integrity failures do not silently downgrade to plaintext storage, and records written by an unknown future version are left untouched and rejected.

Logout writes a secret-free marker under an independent native-vault account when available, keeps the secret-free `oauth.json.logout` and `oauth.json.state` markers, then attempts and verifies deletion from both credential locations even if a marker write fails. Every later load honors either marker and retries cleanup; only a newly-authorized device session may retire a completed logout tombstone. If no marker can be persisted while deletion also remains unconfirmed, the running Qt application refuses normal window, tray, and signal-driven exit until an explicit retry reaches a safe state.

A hard process kill or total storage failure can bypass that in-process close inhibition; software cannot promise durable logout when every independent marker write and credential deletion fails. In that boundary case, keep the process running and retry logout after storage recovers rather than force-killing and relaunching it. Keep `cookies.jar` and any fallback `oauth.json` safe: either can contain authorization material that gives access without the account password. Mutable application data is stored per user in `%LOCALAPPDATA%\\TwitchDropsMiner` on Windows, `${XDG_DATA_HOME:-~/.local/share}/TwitchDropsMiner` on Linux, and `~/Library/Application Support/TwitchDropsMiner` on macOS.
